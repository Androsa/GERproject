...
// libera uma interface. Feito para rodar somente se iface for uma bridge
// (i.e. para uso em um switch linux com Linux bridge)
void authorize_port(struct sta_info * sta, char * iface) {
  char cmd[128];

  snprintf(cmd, 128, "hostapd_authorize_port -p %x:%x:%x:%x:%x:%x %s", sta->addr[0], \
            sta->addr[1], sta->addr[2], sta->addr[3], sta->addr[4], sta->addr[5], iface);
  system(cmd);
}


// bloqueia uma interface. Feito para rodar somente se iface for uma bridge
// (i.e. para uso em um switch linux com Linux bridge)
void unauthorize_port(struct sta_info * sta, char * iface) {
  char cmd[128];

  snprintf(cmd, 128, "hostapd_authorize_port -b %x:%x:%x:%x:%x:%x %s", sta->addr[0], \
            sta->addr[1], sta->addr[2], sta->addr[3], sta->addr[4], sta->addr[5], iface);
  system(cmd);
}
				   ...
				   struct sta_info *sta, int authorized)
{
	...
	if (authorized) {
		...
		res = hapd->drv.set_authorized(hapd, sta, 1);
                // chama o liberador de porta ... 
                authorize_port(sta, hapd->iconf->bss[0].iface);
		...
		res = hapd->drv.set_authorized(hapd, sta, 0);
                // chama o bloqueador de porta ... 
                unauthorize_port(sta, hapd->iconf->bss[0].iface);
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
...
int ieee802_1x_init(struct hostapd_data *hapd)
{
	...
	printf("@@@@@@@@ initializing ...\n");
        ...
        system("hostapd_authorize_port -i");
...
void ieee802_1x_deinit(struct hostapd_data *hapd)
{
	...
	system("hostapd_authorize_port -a");
